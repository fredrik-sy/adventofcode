...
def connect(x, y, depth):
    if isinstance(x, list):
        x = connect(x.pop(0), x.pop(), depth + 1)
    if isinstance(y, list):
        y = connect(y.pop(0), y.pop(), depth + 1)
...
with open('input.txt') as io:
    ...
    connected = connect(*numbers.pop(0), depth=1)
    while len(numbers) > 0:
        for number in connected.all():
            number.depth += 1
        connected = Point(connected, connect(*numbers.pop(0), depth=2), 1)
        reduce(connected)
    print(connected.magnitude())
